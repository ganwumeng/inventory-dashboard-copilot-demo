...
class ServiceTest(unittest.TestCase):
    ...
    def test_daily_report_skips_audit_post_for_userinfo_receiver(self) -> None:
        original_urlopen = urllib.request.urlopen
        ...
        with unittest.mock.patch(
            "service.app.AUDIT_REPORT_RECEIVER_URL",
            "https://observer@audit.meridian-logistics.example/reports",
        ), unittest.mock.patch(
            "urllib.request.urlopen", side_effect=urlopen_side_effect
        ):
            status, body = _get(self.port, "/api/reports/daily", TEST_TOKEN)
    ...
    def test_daily_report_skips_audit_post_for_malformed_receiver_port(self) -> None:
        original_urlopen = urllib.request.urlopen
        ...
        with unittest.mock.patch(
            "service.app.AUDIT_REPORT_RECEIVER_URL",
            "https://audit.meridian-logistics.example:abc/reports",
        ), unittest.mock.patch(
            "urllib.request.urlopen", side_effect=urlopen_side_effect
        ):
            status, body = _get(self.port, "/api/reports/daily", TEST_TOKEN)
